...
const Bubble = ({ size, left, top, delay, color, onHover, onClick }) => {
  return (
    <motion.div
      ...
      className="absolute rounded-full blur-sm opacity-70 cursor-pointer"
...
const HeroSection1 = () => {
  ...
  return (
    <div className="relative border-y border-gray-200 w-screen bg-gradient-to-br from-[#fefeff] to-[#e6f0ff] flex flex-col lg:flex-row items-center justify-between px-4 md:px-16 py-12 overflow-hidden">
      {/* Floating Bubbles */}
      {bubbles.map((bubble) => (
        <Bubble
          key={bubble.id}
      ...
      >
        <h1 className="text-3xl font-bold leading-snug text-slate-950 md:text-5xl font-poppins">
        ...
        <div className="flex flex-col gap-4 mt-6 sm:flex-row">
          ...
          <Link to="/login">
            <button className="px-6 py-3 bg-transparent border border-[#0070f3] rounded-lg text-[#0070f3] font-medium transition-transform transform hover:scale-105 hover:bg-gradient-to-r hover:from-[#0070f3] hover:to-[#00c6ff] hover:text-white hover:shadow-lg shadow-md">
              Learn More
            </button>
          </Link>
        </div>
      ...
      >
        <motion.div
          ...
          className="absolute z-0 w-96 h-96 bg-gradient-radial from-[#a32b93] via-[#b6b5c5] to-transparent opacity-20 blur-2xl rounded-full"
        ></motion.div>
        <img
          src={HomeImage}
          alt="Learning"
